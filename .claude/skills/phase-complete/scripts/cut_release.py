'''phase-complete Part B — cut a release (mechanical bookkeeping).
...
def cut(args: argparse.Namespace) -> dict:
    ...
    attr = re.escape(cfg['version_attr'])
    nvtext = re.sub(attr + r"(\s*=\s*)['\"][^'\"]+['\"]", rf"{cfg['version_attr']}\g<1>'{version}'", vtext, count=1)
    edits[vf] = nvtext
